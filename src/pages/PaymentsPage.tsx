...
export default function PaymentsPage() {
  ...
  const { data: payments = [], isLoading } = useTableQuery<any>('payments');

  const filtered = payments.filter((p: any) =>
    (p.pay_no || '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="erp-page">
      <div className="erp-page-header">
        <div>
          <h1 className="erp-page-title flex items-center gap-2"><CreditCard className="w-6 h-6 text-primary" />Payments</h1>
          <p className="erp-page-subtitle">{payments.length} payments — AR & AP with FX gain/loss</p>
        </div>
      </div>
                ...
                {filtered.length === 0 ? (
                  ...
                  <tr key={p.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right">
                      <span className={(p.fx_gain_loss_usd || 0) > 0 ? 'fx-gain' : (p.fx_gain_loss_usd || 0) < 0 ? 'fx-loss' : 'text-muted-foreground'}>
                        {(p.fx_gain_loss_usd || 0) > 0 ? '+' : ''}{(p.fx_gain_loss_usd || 0) !== 0 ? `$${p.fx_gain_loss_usd.toFixed(2)}` : '—'}
                      </span>
                    </td>
