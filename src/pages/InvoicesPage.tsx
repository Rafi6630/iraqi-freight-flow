  ...
  return (
    <div className="erp-page">
      <div className="erp-page-header">
        <div>
          <h1 className="erp-page-title flex items-center gap-2"><FileText className="w-6 h-6 text-primary" />Invoices</h1>
          <p className="erp-page-subtitle">{invoices.length} invoices — USD | IQD</p>
        </div>
      </div>
        ...
        <div className="erp-table-container">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="px-5 py-8 text-center text-muted-foreground">No invoices yet. Generate from Order Wizard Step 7.</td></tr>
                  ...
                  <tr key={inv.id} className="border-b border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3">{customers.find((c: any) => c.id === inv.customer_id)?.company || '—'}</td>
                    ...
                    <td className="px-5 py-3 text-right"><CurrencyDisplay usd={inv.paid_usd || 0} iqd={inv.paid_iqd || 0} size="sm" /></td>
                    <td className="px-5 py-3 text-muted-foreground">{inv.due_date || '—'}</td>
                    <td className="px-5 py-3 text-center">{inv.is_fx_locked && <FxLockedBadge />}</td>
                  </tr>
                ))}
              </tbody>
            </table>
